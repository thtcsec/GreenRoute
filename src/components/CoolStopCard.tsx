          ...
          {coolstops.map((stop) => {
            ...
            return (
              <div
                ...
                className={`min-w-[140px] p-2.5 rounded-xl bg-gray-950 border cursor-pointer transition-all duration-200 shrink-0 ${
                  isActive
                    ? 'border-emerald-500 shadow-md shadow-emerald-500/10'
                    : 'border-gray-800 hover:border-gray-700'
                }`}
              >
                <p className={`text-xs font-semibold truncate ${isActive ? 'text-emerald-400' : 'text-white'}`}>
                  {stop.name}
                </p>
                <p className="text-[10px] text-gray-500 mt-0.5">{stop.distance}m</p>
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            {recommendedStop.operatingHours && (
              <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[10px] font-medium bg-gray-800 text-gray-300 border border-gray-700">
                🕐 {recommendedStop.operatingHours}
              </span>
            )}
          </div>
      ...
      <div className="border-t border-gray-850 pt-4 mb-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs bg-red-950/30 border border-red-950 px-3 py-2 rounded-lg text-red-300">
            ...
            <span className="font-bold text-red-400">Shade Score: 2/10, Không mái che</span>
          </div>
          <div className="flex items-center justify-between text-xs bg-emerald-950/40 border border-emerald-900/60 px-3 py-2 rounded-lg text-emerald-300">
            ...
            <span className="font-bold text-emerald-400">Shade: {recommendedStop.shadeScore}/10, {recommendedStop.amenities[0] || 'Có bóng râm'}</span>
          </div>
        </div>
      </div>
